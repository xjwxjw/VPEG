...
class SequenceDataLoader(BaseDataLoader):
    ...
    def __init__(self, data_dir, subset,
        ...
        # load NN example index
        self.ref_data = dict()
        if self.subset =='train':
            self.idx_dir = ['./Penn_Action/NN/Train/', './Penn_Action/NN/Test/'] 
        else:
            self.idx_dir = ['./Penn_Action/NN/Test/']
        for idx_subdir in self.idx_dir:
            for filename in os.listdir(idx_subdir):
                self.ref_data[filename] = io.loadmat(osp.join(idx_subdir, filename))
        
        # load all data

        self.seq_data = dict()
        self.mean_data = dict()
        for filename in os.listdir('./Penn_Action/pseudo_labels/'):
            cur_seq = np.load(osp.join('./Penn_Action/pseudo_labels/', filename))
            # print(cur_seq.shape)
            mean_seq = np.mean(cur_seq, 1, keepdims=True)
            # cur_seq -= mean_seq
            self.seq_data[filename] = cur_seq
            self.mean_data[filename] = mean_seq


        self._total = len(self._images) * self.n_samples
    ...
    def get_sample_shape(self):
        shape_dict = {
            ...
            'mean_seq': [N_SEQUENCE_LEN - 1, 1, 2],
            'ref_seq': [5, N_SEQUENCE_LEN - 1, self.n_points, 2],
    ...
    def sample_generator(self):
        if self._random_order:
            ...
            for idx in range(self._total):
                yield self._get_image_at(idx// self.n_samples)
    ...
    def _get_image_at(self, idx):
        ...
        mean_keypoints = self.mean_data[img_path.replace('frames/', '') + '.npy']
        keypoints = self.seq_data[img_path.replace('frames/', '') + '.npy']
        ref_data = self.ref_data[img_path.replace('frames/', '') + '.mat']
        # if self.subset =='train':
        #     ref_data = io.loadmat(osp.join(self._data_dir, img_path.replace('frames', 'NN/Train') + '.mat'))
        # if self.subset =='test':
        #     ref_data = io.loadmat(osp.join(self._data_dir, img_path.replace('frames', 'NN/Test') + '.mat'))
        ...
        if self._randomness:
            ...
            im_idx = 0
        # print(img_path, im_idx)
        # load image
        image = self._load_image(img_path, im_idx)

        # load real keypoints sequence
        if gap >= 1:
            fr_idx = [(im_idx + gap * i) % file_len for i in range(N_SEQUENCE_LEN)]
            real_seq = keypoints[fr_idx, :, :]
            mean_seq = mean_keypoints[fr_idx, :, :]
            ...
            mean_seq = np.zeros([N_SEQUENCE_LEN, 1, 2])
            half_mean_seq = mean_keypoints[im_idx:im_idx + n_seq, :, :]
            for i in range(n_seq - 1):
                mean_seq[i * 2] = half_mean_seq[i]
                mean_seq[i * 2 + 1] = (half_mean_seq[i] + half_mean_seq[i + 1]) / 2.0
                pass
            mean_seq[-1] = half_mean_seq[-1]
        
        ref_seq = []
        cur_j = 0
        for j in range(5):
            if float(ref_data[str(im_idx)][j][1]) < 10:
                ref_filename = ref_data[str(im_idx)][j][0].replace('frames/', '').split('_')
                cur_j = j
            else:
                ref_filename = ref_data[str(im_idx)][cur_j][0].replace('frames/', '').split('_')
            # ref_keypoints = np.load(self._data_dir + '/pseudo_labels' + ref_filename[0] + '.npy')
            ref_keypoints = self.seq_data[ref_filename[0] + '.npy']
            fr_idx = [int(ref_filename[1]) + i for i in range(N_SEQUENCE_LEN)]
            # print(j, ref_keypoints.shape[0], int(ref_filename[1]) + 32, )
            ref_seq.append(ref_keypoints[fr_idx, :, :])
        ...
        if self._randomness:
            ...
            for j in range(5):
                ref_seq[j] = data_utils.rotate_keypoints(ref_seq[j], rotation_rand_val)
            pass
        ...
        if self._with_image_seq:
            ...
            for i in range(1, n_future_frames + 1):
                current_im = self._load_image(img_path, (im_idx + gap * i) % file_len)
