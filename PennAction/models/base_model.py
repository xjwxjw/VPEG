...
class BaseModel(ABC):
    ...
    def initialize_loggers(self, log_dir, sess, now):
        """
        ...
        if self.is_training:
            self.train_writer = tf.summary.FileWriter(osp.join(log_dir, name, 'train', now), sess.graph)
            self.test_writer = tf.summary.FileWriter(osp.join(log_dir, name, 'test', now))
