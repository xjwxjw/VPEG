...
def vae_encoder(x, f_pt, act_code, cell_info, vae_dim, ref_seq = None):
    with tf.variable_scope('vae_encoder'):
        ...
        if ref_seq is None:
            ...
            outputs, _ = tf.nn.dynamic_rnn(cell, ref_seq[:, 0], initial_state=state, dtype=tf.float32)
            cur_ref = tf.concat([ref_seq[:, j, 0] for j in range(5)], -1)
            logit = tf.contrib.layers.fully_connected(tf.concat([outputs[:, -1, :], f_pt, act_code, cur_ref], axis=-1), vae_dim * 2)
        mu = logit[:, :vae_dim]
        stddev = logit[:, vae_dim:]
        return mu, stddev


def vae_decoder(x, f_pt, act_code, cell_info, vae_dim, n_pts, ref_seq = None):
    with tf.variable_scope('vae_decoder', reuse=tf.AUTO_REUSE):
        cell = layers.lstm_model(cell_info)
        state = cell.zero_state(tf.shape(x)[0], tf.float32)
        if not (ref_seq is None): 
            input_ = tf.contrib.layers.fully_connected(tf.concat([x, f_pt, act_code], axis=-1), 512)
        else:
        ...
        for i in range(31):
            if not (ref_seq is None):
                cur_ref = tf.concat([ref_seq[:, j, i+1] for j in range(5)], -1)
                input_ref = tf.contrib.layers.fully_connected(tf.concat([x, cur_ref, act_code], axis=-1), 512)
                output, state = cell(input_ref, state)
            else:
